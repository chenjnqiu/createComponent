import React, { createContext, forwardRef, useContext, useRef, useCallback, cloneElement, Children, Component, useMemo, PureComponent } from 'react';
...
class Badge extends PureComponent {
    ...
    renderCount = () => {
        ...
        if (dot) {
            return (React.createElement("span", { className: 'cjq-badge-dot', style: badgeStyle }));
        }
        else if (count > 0 || (count === 0 && showZero)) {
            return (React.createElement("span", { className: 'cjq-badge-count', style: badgeStyle },
                React.createElement("span", { className: 'cjq-badge-count-num' }, count > maxCount ? `${maxCount}+` : count)));
        }
        return null;
    };
    render() {
        ...
        return (React.createElement("div", { className: containerCls },
            children ? React.createElement("div", { className: 'cjq-badge-content' }, children) : null,
            this.renderCount()));
    }
}
...
const Tag = forwardRef(({ size = 'small', theme = 'grey', outline, rounded = true, closable, children, className, onClose, closeButtonStyle, visible = true, ...rest }, ref) => {
    if (!visible) {
        return null;
    }
    ...
    return (React.createElement("div", { ref: ref, className: classnames$1, ...rest },
        React.createElement("div", { className: 'cjq-tag-content' }, children),
        closable ? (React.createElement(Icon, { type: "closed", className: "cjq-tag-close-btn", onClick: onClose, style: closeButtonStyle })) : null));
});
Tag.displayName = 'Tag';

const LinkTag = forwardRef(({ className, children, linkIconStyle, ...rest }, ref) => {
    return (React.createElement(Tag, { className: `cjq-link-tag ${className}`, ref: ref, outline: true, ...rest },
        React.createElement("div", { className: "cjq-link-tag-content" }, children),
        React.createElement(Icon, { type: "arrow-right", className: "cjq-link-tag-right-icon", style: linkIconStyle })));
});
LinkTag.displayName = 'LinkTag';

const SelectTag = forwardRef(({ className, children, selected, onChange, ...rest }, ref) => {
    ...
    return (React.createElement(Tag, { className: classNames, ref: ref, onClick: handleClick, outline: true, ...rest }, children));
});
SelectTag.displayName = 'SelectTag';

export { Badge, Button, Checkbox, DisabledContext, Icon, Link, LinkTag, Radio, SelectTag, Switch, Tag };
